...
def main():
    env = PandaReachEnv(randomize_target=False)
    ...
    assert env.action_space.shape == (6,), "Reach action must be 6D."
    ...
    for step in range(10):
        ...
        print(
            f"step={step + 1:02d}",
            f"reward={reward:.4f}",
            f"pos_dist={info['position_distance']:.4f}",
            f"ori_dist={info['orientation_distance']:.4f}",
