...
def main():
    env = PandaGraspEnv(randomize_object=True)
    ...
    assert env.action_space.shape == (7,), "Grasp action must be 7D."
    ...
    for step in range(10):
        ...
        print(
            f"step={step + 1:02d}",
            f"reward={reward:.4f}",
            f"reach={info['reach_distance']:.4f}",
            f"lift={info['lift_height']:.4f}",
